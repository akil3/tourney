  ...
  if (!stats) return (
    <div className="flex items-center justify-center py-20">
      <div className="text-center">
        ...
        <p className="text-[var(--text-muted)] text-[14px]">Loading dashboard...</p>
      </div>
    </div>
  );
      ...
      {stats.liveMatches > 0 && (
        <div className="bg-[var(--accent)] rounded-2xl p-4 overflow-hidden relative">
          <div className="absolute top-3 right-4 flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-bold text-white/80 uppercase tracking-widest">Live Now</span>
          </div>
          <div className="flex gap-6 overflow-x-auto pb-1">
            {stats.liveMatchDetails.map(m => {
              ...
              return (
                <div key={m.id} className="bg-white/10 backdrop-blur rounded-xl px-4 py-3 min-w-[220px] shrink-0">
                  <p className="text-[10px] font-bold text-white/60 uppercase tracking-widest mb-2">{m.tournament.name}</p>
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-[14px] font-semibold text-white truncate">{m.homeTeam?.name ?? "TBD"}</span>
                    <span className="font-display text-[24px] text-white tracking-wider">{currentSet ? `${currentSet.home}:${currentSet.away}` : "0:0"}</span>
                    <span className="text-[14px] font-semibold text-white truncate">{m.awayTeam?.name ?? "TBD"}</span>
                  </div>
                  <p className="text-[10px] text-white/50 text-center mt-1">Set {sets.length || 1}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="relative overflow-hidden">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.15em]">Active</p>
              <p className="font-display text-[56px] leading-none tracking-wider text-[var(--accent)] mt-1">{stats.activeTournaments}</p>
              <p className="text-[12px] text-[var(--text-muted)] mt-1">{stats.draftTournaments} draft &middot; {stats.completedTournaments} done</p>
            </div>
        ...
        <Card>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.15em]">Matches</p>
              <p className="font-display text-[56px] leading-none tracking-wider text-[var(--text-primary)] mt-1">{stats.matchCount}</p>
              <p className="text-[12px] text-[var(--text-muted)] mt-1">{stats.liveMatches} live &middot; {stats.pendingMatches} pending</p>
            </div>
        ...
        <Card>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.15em]">Teams</p>
              <p className="font-display text-[56px] leading-none tracking-wider text-[var(--text-primary)] mt-1">{stats.teamCount}</p>
              <p className="text-[12px] text-[var(--text-muted)] mt-1">{stats.playerCount} players</p>
            </div>
            <div className="w-10 h-10 rounded-xl bg-[var(--bg-muted)] flex items-center justify-center">
        ...
        <Card>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-[0.15em]">Users</p>
              <p className="font-display text-[56px] leading-none tracking-wider text-[var(--text-primary)] mt-1">{stats.userCount}</p>
              <p className="text-[12px] text-[var(--text-muted)] mt-1">registered accounts</p>
            </div>
            <div className="w-10 h-10 rounded-xl bg-[var(--bg-muted)] flex items-center justify-center">
        ...
        <Card className="!p-3 sm:!p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] font-bold text-[var(--text-muted)] uppercase tracking-[0.15em]">Match Completion</span>
            </div>
        ...
        <div className="lg:col-span-3">
          <Card>
            ...
            {stats.recentMatches.length === 0 ? (
              <p className="text-[14px] text-[var(--text-muted)] py-8 text-center">No completed matches yet</p>
            ) : (
                    ...
                    <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--bg-muted)] hover:bg-[var(--bg-base)] transition-colors">
                      ...
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <div className="flex-1 min-w-0 text-right">
                          <p className={`text-[14px] truncate ${homeWon ? "font-bold text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`}>{m.homeTeam?.name ?? "TBD"}</p>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <span className={`font-display text-[24px] tracking-wider ${homeWon ? "text-[var(--accent)]" : "text-[var(--text-muted)]"}`}>{m.homeScore}</span>
                          <span className="text-[var(--text-muted)] text-[12px]">:</span>
                          <span className={`font-display text-[24px] tracking-wider ${!homeWon ? "text-[var(--accent)]" : "text-[var(--text-muted)]"}`}>{m.awayScore}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`text-[14px] truncate ${!homeWon ? "font-bold text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`}>{m.awayTeam?.name ?? "TBD"}</p>
                        </div>
                      </div>
                      {/* Set scores */}
                      <div className="hidden sm:flex gap-1 shrink-0">
                        {sets.map((s, i) => (
                          <span key={i} className="text-[10px] font-mono text-[var(--text-muted)] bg-[var(--bg-surface)] px-1.5 py-0.5 rounded">
                            {s.home}-{s.away}
                          </span>
                        ))}
                      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/admin/tournaments" className="text-[12px] text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors">View all →</Link>
            </div>
            {stats.tournaments.length === 0 ? (
              <p className="text-[14px] text-[var(--text-muted)] py-4 text-center">No tournaments</p>
            ) : (
              <div className="space-y-2">
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/admin/users" className="text-[12px] text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors">View all →</Link>
            </div>
            {stats.recentUsers.length === 0 ? (
              <p className="text-[14px] text-[var(--text-muted)] py-4 text-center">No users</p>
            ) : (
              <div className="space-y-1">
                {stats.recentUsers.map(u => (
                  <div key={u.id} className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-7 h-7 rounded-lg bg-[var(--bg-muted)] flex items-center justify-center text-[var(--text-muted)] font-display text-[14px] tracking-wider shrink-0">
          ...
          <Card className="!p-3 sm:!p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[var(--bg-muted)] rounded-xl p-3 text-center">
                ...
                <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">Avg Teams/Tourney</p>
              </div>
              <div className="bg-[var(--bg-muted)] rounded-xl p-3 text-center">
                <p className="font-display text-[28px] tracking-wider text-[var(--text-primary)] leading-none">{stats.playerCount}</p>
                <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">Total Players</p>
              </div>
            </div>
          </Card>
        </div>
